...
import { Lotto36Numbers } from '../value-objects/lotto36-numbers.vo';

export interface ILotto36Props {
  readonly id: string;
  roundNumber: RoundNumber;
  drawnNumbers: Lotto36Numbers | null;
  sum: number | null;
  drawnAt: Date | null;
  status: LottoStatus;
  overUnder: Lotto36OverUnder | null;
  range: Lotto36Range | null;
  isJackpot: boolean;
  jackpotOn: Lotto36Range | null;
  closedAt: Date | null;
...
// Aggregate Root
export class Lotto36 {
  private props: ILotto36Props;

  constructor(props: ILotto36Props) {
    this.props = props;
  }
  ...
  static reconstitute(props: ILotto36Props): Lotto36 {
    return new Lotto36(props);
  }
  ...
  get drawnNumbers(): Lotto36Numbers | null {
  ...
  get overUnder(): Lotto36OverUnder | null {
    return this.props.overUnder;
  }

  get range(): Lotto36Range | null {
  ...
  get jackpotOn(): Lotto36Range | null {
  ...
  startDrawing(): void {
    ...
    this.props.drawnNumbers = new Lotto36Numbers([]);
  ...
  addBall(ball: number): void {
    ...
    // Add the new ball
    this.props.drawnNumbers = new Lotto36Numbers([...currentNumbers, ball]);
    this.props.updatedAt = new Date();
    ...
    if (this.props.drawnNumbers.getCount() === 20) {
      ...
      this.props.overUnder =
        this.props.sum > LOTTO36_OVER_UNDER_THRESHOLD
          ? Lotto36OverUnder.OVER
          : Lotto36OverUnder.UNDER;
      this.props.range = sumLotto36Range(this.props.sum);
    }
  }
   ...
   */
  setJackpotOn(range: Lotto36Range): void {
